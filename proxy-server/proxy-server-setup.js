...
const pServerAuthBasic = require ('./pserver-basic-auth.js');
...
module.exports = proxyServerSetup;

// -> main -> gets an already created http.server instance + conf
function proxyServerSetup (server) {

	if (!server) { 
		
		// console.log('web-server not pre found. creating it.')
		
		server = http.createServer(function (req, res) {
			
			if (res && res.header && typeof res.header === 'function') {
	...
	server.agent = conf.agent;
	server.localAddress = conf.localAddress;
	server.mustAuthenticate = true;

	// console.log('server.agent', server.agent)
	// console.log('server.localAddress', server.localAddress)
	// console.log('server.mustAuthenticate', server.mustAuthenticate)
...
function authenticate (server, req, fn) {
	...
	// if HAS AUTHENTICATE and is a function
	if (server.mustAuthenticate) {
	
		debug.request('authenticating request "%s %s"', req.method, req.url);
		server.authenticate = pServerAuthBasic;
		server.authenticate(req, fn)

	} else { // no `server.authenticate()` function, so just allow the request
		fn(null, true);
	}
}
...
function requestAuthorization(req, res) {
	...
	debug.response('requesting proxy authorization for "%s %s"',
						req.method, req.url);
	let realm = 'proxy';
	let headers = {
